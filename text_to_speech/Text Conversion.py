...
def generate_audio(our_data, voices_dir, save_to_dir):
...
def make_audiobook(text, voices_directory, generation_space, output_directory, name_of_output):
    if not check_voices(text, voices_directory):
        ...
        generate_audio(text, voices_directory, generation_space)
...
# replace this with our NLP functions
test_data = pd.read_csv('book_data3.csv')
data = []
for s, v in zip(test_data.iloc[:, 0], test_data.iloc[:, 1]):
    data.append((s, v))
...
# make_audiobook(data, 'voices', 'input_dir', 'output_dir', 'riddles')
# overlay_music('output_dir/riddles.wav', 'music/gollum_music.wav', 'riddles_with_music.wav')
